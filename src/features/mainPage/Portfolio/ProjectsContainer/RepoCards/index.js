...
export const RepositoryDisplayItem = ({ repoData }) => {
  ...
  const checkIfWebApplication = (repositoryName, repositoryDescription) => {
    ...
    const combinedText = `${repositoryName} ${repositoryDescription || ''}`.toLowerCase();
    const hasWebKeywords = webProjectKeywords.some(keyword => combinedText.includes(keyword));
  ...
  if (!liveUrl && !isFork && !isPrivateRepo && repoOwner?.login) {
    const shouldGenerateUrl = hasGitHubPages || checkIfWebApplication(repositoryName, repoDescription);
